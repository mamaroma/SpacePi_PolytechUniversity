...
Ключ сообщества хорошо публикует текст. Для фото VK часто запрещает `photos.*`
с community-токеном — код пробует по очереди:

1. загрузку фото на стену;
2. загрузку картинок как **документов** стены (нужно право «Документы» у ключа);
3. вложение публичных URL картинок + ссылки на новость.

Если фото всё ещё не видны: пересоздайте ключ сообщества с правами
**Стена** + **Документы** + **Фотографии** (если есть в списке).
